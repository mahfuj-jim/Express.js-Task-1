  ...
  try {
    ...
    if (verifyToken) {
      if (verifyToken.role === "user" || verifyToken.role === "admin") {
        ...
        verifyToken.role === "restaurant" &&
        verifyToken.restaurant._id === restaurantId
      ) {
        next();
      } else {
        return failure(
...
function validateOrderConfirmByRestaurantToken(req, res, next) {
  const authHeader = req.header("Authorization");
  ...
  try {
    ...
    if (verifyToken) {
      if (verifyToken.role === "admin") {
        next();
      } else if (verifyToken.role === "restaurant") {
        next();
      } else {
        return failure(
          res,
...
function validateOrderReachByRiderToken(req, res, next) {
  const authHeader = req.header("Authorization");
